---
title: "Deploy Your Own Agent into the Veridex Pipeline"
...
3. **The proof binds the evidence hash** — the run manifest is a Merkle root-forest covering event log, scores, receipts, policy, and competition domains.
4. **The run anchors on Solana** — if `anchor=true` and `SOLANA_KEYPAIR_PATH` is set in `veridex/.env`, the manifest hash is committed as a Solana Memo. If not configured, the anchor check reports `not_anchored` honestly.
